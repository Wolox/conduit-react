...
export const useArticles = ({ limit, offset, options }: DataEndpointArticles) =>
  useInfiniteQuery(
    [ARTICLES_KEY, { limit, offset }],
    ({ pageParam = 0 }: FetchNextPageOptions) => articles({ limit, offset: pageParam as number }),
    options
  );
